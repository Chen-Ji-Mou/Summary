...
| 使用场景 | 少量字符串操作 | 多线程下的大量字符串操作 | 单线程下的大量字符串操作 |

[String、StringBuffer、StringBuilder的区别——（面试题）](https://blog.csdn.net/Turniper/article/details/111112824)

### String 通过 "+" 连接和通过 StringBuilder 连接有什么区别？

本质上没有区别

在 Java 源代码中通过 "+" 连接 String 字符串，经过编译后，在字节码中仍是通过 StringBuilder 连接 String 字符串

[StringBuilder与“+”加号拼接字符串区别](https://blog.csdn.net/wang_chao_yang/article/details/106934101)
...
* 方法重写：同一个父类的方法可以根据实例化子类的不同有不同的实现
* 对象转型：一个对象可以向上转型成父类类型，也可以向下转型成子类类型

### 什么是方法的重写？那方法的重载呢？

方法的重写是指同一个父类的方法可以根据实例化子类的不同有不同的实现

方法的重载是指同一个方法名称可以根据参数的类型或个数不同调用不同的方法体

### 抽象类和接口的区别？

* 接口中只能有方法的定义，不能有方法的实现 (在 JDK 1.8 版本接口中可以定义 default 方法体)
...
* 接口中的成员变量默认为 public static final，必须赋初值，不能被修改

  抽象类中的成员变量可在子类中被重新定义，也可被重新赋值
...
在 JVM 中绝大部分内存都属于运行时数据区

运行时数据区结构如下图所示：

![](https://note.youdao.com/yws/api/personal/file/WEB22b464d6ee5df2e8008344b8e80b76cf?method=download&shareKey=f951a9180a73d17abc064e56e8a12010)

在运行时数据区中，主要分为方法区、堆、虚拟机栈、本地方法栈、程序计数器等区域

方法区和堆是线程共享的，而虚拟机栈、本地方法栈、程序计数器这三个是线程私有的

虚拟机栈结构如下图所示：

![](https://note.youdao.com/yws/api/personal/file/WEBf8270e35ba6158e6e845670d1780c793?method=download&shareKey=6606dba40bf9d549afcc699a65aa9c67)

虚拟机栈是基于线程的，所有的方法都是以线程的方式运行的

在线程的生命周期中，参与计算的数据会频繁地入栈和出栈，栈的生命周期是跟随线程的

虚拟机栈中储存的数据结构我们称之为栈帧

当每个 Java 方法被调用时，都会创建一个栈帧，并入栈；当方法执行完成后，则出栈
...
堆是 JVM 上最大的内存区域，我们创建的几乎所有的对象，都是在这里存储的，GC 操作的对象就是堆

方法区中储存的数据主要包括：类的信息、常量池、方法数据、方法代码等

# 计算机网络

## Http

### 在浏览器中输入网址到整个页面展示的大致过程？

1. 访问 DNS 服务器解析域名，获取对应的 ip 地址
2. 建立 TCP 连接 (三次握手)
3. 浏览器发送 Http 请求
4. 服务器处理请求，返回 Http 报文
5. 浏览器解析返回的数据并渲染页面
6. 断开 TCP 连接 (四次挥手)
...
进程是系统资源分配的基本单元，每个进程都有自己独立的一块内存空间 (用户态)，一个进程中可以有多个线程，一个进程至少有一个线程，同一进程中的多个线程可共享数据

线程是程序执行的最小单元，负责当前进程中程序的执行

### 进程的内存模型 (虚拟内存) ？

现在操作系统中的进程都是采用 **虚拟存储器 (虚拟内存)**，对于 32 位系统而言，它的寻址空间 (虚拟存储空间) 就是 2 的 32 次方，也就是 4GB
...
针对 Linux 操作系统而言，将地址位最高的 1GB 字节供内核使用，称为 **内核空间**；地址位较低的 3GB 字节供各进程使用，称为 **用户空间**

内存模型大致如下图所示：

![](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/39ecb72468f7482293e1ac5b11662dda~tplv-k3u1fbpfcp-watermark.awebp)

### 常见的进程调度策略？



### 为什么并发常通过多线程实现，而不是通过多进程？
...
* 初始 (new)：线程对象已经被创建，但线程还没有开始运行

* 运行 (runnable)：Java 线程中将就绪 (ready) 和运行中 (running) 两种状态笼统的称为 “运行”

  该状态的线程位于可运行线程池中，等待被线程调度选中，获取 CPU 的使用权，此时线程处于就绪态 (ready)

  当处于就绪态的线程获得 CPU 时间片后变为运行中状态 (running)

* 阻塞 (blocked)：表示线程阻塞于锁，此时线程处于内核态
* 等待 (waiting)：处于该状态的线程需要等待其他线程做出特定动作 (通知或中断)
* 超时等待 (timed_waiting)：该状态不同于等待态，处于该状态的线程可以在指定的时间后自行返回
